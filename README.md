...
**The local demo mode** (`/demo`, gated behind `VITE_DEMO_MODE`, on by default in local dev and off in a real production build unless explicitly enabled) is a separate, deliberate thing: no login, no backend, everything in the browser's own `sessionStorage`, seeded from the generated Sahaj Residency dataset in `src/lib/demoSeed.ts` (not from `sample-data/`, which is a different thing, the real app's own local-fallback seed - see section 6). Useful for exploring the UI, showing someone the product without setting anything up, or local development without a Supabase project connected. Refreshing keeps demo changes within the same tab; closing the tab, or opening a different browser, resets to that starting seed. A permanent **ડેમો સોસાયટી** identity banner sits at the top of every demo screen with a **રીસ્ટાર્ટ** control that clears the session straight back to the starting seed, reachable from anywhere in the demo rather than only from the committee panel's settings. Nothing in demo mode is ever sent anywhere, and nothing from a demo session can end up in a real one or vice versa - they're entirely separate data layers, not one falling back to the other.
...
`supabase/schema.sql` is the canonical schema: every table, indexes, Row Level Security policies covering every real tenant boundary this app actually needs, storage bucket policies, and the database functions RLS depends on. It is the single fresh-install target, kept in step with the additive migrations under `supabase/migrations/` so a brand-new project built from it and an existing one brought forward by the migrations end up identical. It is not a snapshot of whatever is currently applied to the live production database, which this repo cannot see or verify on its own (see `PRODUCTION-RUNBOOK.md` section 2). `supabase/README.md` covers project setup, environment variables, the free tier's inactivity pause and the fix for it, and deploying to Cloudflare Pages. `supabase/tests/run-isolation-tests.sh` is a repeatable, automated check of the actual tenant isolation this schema provides - two societies, every role, run against a fresh throwaway database - not something that has to be re-verified by hand after every change.
...
- **All data in the local demo is invented.** The names, generated phone numbers, and amounts in the demo's seed (`src/lib/demoSeed.ts`, the fictional Sahaj Residency society) are for demonstration only, and never appear in a real session.
...
sample-data/      real app's local-fallback seed (used only when Supabase isn't configured), JSON
