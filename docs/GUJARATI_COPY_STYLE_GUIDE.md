...
- **If a button turns into a sentence, move the detail into helper text** below it.
  Worked examples of the setup-request CTA:
  - **Features** uses the short **`સેટઅપ વિનંતી કરો`** CTA, with the full invitation
    kept as nearby supporting text.
  - **Pricing** keeps the full **`સોસાયટી સેટઅપની વિનંતી કરો`** CTA — its explanatory
    note already supplies the context, so shortening it there is neither needed nor
    part of the audit.
  - **Owner onboarding** has no matching setup-request CTA (the owner creates the
    society directly rather than requesting setup); its step controls already use
    short action verbs such as `આગળ` and `સોસાયટી સક્રિય કરો`, so no copy change was
    required there.
