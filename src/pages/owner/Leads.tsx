...
export default function OwnerLeads() {
  ...
  // db.leads is already populated for real by the owner's own shared
  // fetch effect (see store.tsx) - this used to duplicate that with its
  // own separate fetch and its own separate local state, which is also
  // exactly why its own status updates were never wired into the same
  // failed-write visibility every other real write in the app has:
  // updateLeadStatus below is the one, shared, already-wired action now,
  // not a second one living only on this page.
  const leads = [...rawDb.leads].sort((a, b) => b.createdAt.localeCompare(a.createdAt))

  const setStatus = (id: string, status: PublicLead['status'], internalNote?: string) => {
