...
describe('public homepage', () => {
  it('renders Gujarati-first hero copy with one primary demo action and a login action when demo mode is enabled', () => {
    ...
    expect(within(main).getByRole('link', { name: 'રહેવાસી લોગિન' })).toHaveAttribute('href', '/user-login')
    expect(within(main).getByRole('link', { name: 'એડમિન લોગિન' })).toHaveAttribute('href', '/admin-login')
    expect(within(main).getByRole('link', { name: 'બિઝનેસ લોગિન' })).toHaveAttribute('href', '/business/login')
