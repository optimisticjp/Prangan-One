...
export default function BusinessStaffPage(){
 ...
 return <div className="space-y-3">
  <div className="flex items-start justify-between gap-2"><div><h1 className="text-[17px] font-bold text-navy-900">Staff money & access</h1><p className="text-[11.5px] text-navy-400">Business money held by staff stays separate from pocket expenses the business owes them.</p></div><button onClick={openAdd} className="h-9 px-3 shrink-0 rounded-xl bg-navy-900 text-white text-[11.5px] font-semibold flex items-center gap-1"><Plus size={14}/> Staff</button></div>
  <div className="rounded-xl bg-navy-50 border border-navy-100 px-3 py-2 text-[11px] text-navy-600">Staff can record how <strong>business money they hold</strong> was spent and what they <strong>paid personally</strong>. Only a partner/admin can issue money, reimburse, pay salary, or settle/return held money.</div>
  <div className="space-y-2">{data.staff.map(staff=>{const p=data.staffPositions.find(x=>x.staff_id===staff.id),held=Number(p?.advance_balance??0),due=Number(p?.outstanding_due??0);return <div key={staff.id} className="rounded-xl border border-cream-200 bg-white p-3">
   ...
   <div className="grid grid-cols-2 gap-2 mt-2.5"><Position icon={WalletCards} label="Business money held" value={held} sub="Still belongs to business" tone="navy"/><Position icon={HandCoins} label="Business owes staff" value={due} sub="Personal pocket expenses due back" tone={due>0?'saffron':'green'}/></div>
   <div className="grid grid-cols-3 gap-1.5 mt-2"><Mini label="Spent from held" value={p?.advance_spent??0}/><Mini label="Pocket expense" value={p?.pocket_expenses??0}/><Mini label="Salary paid" value={p?.salary_paid??0}/></div>
  ...
  <Modal open={!!moneyStaff} onClose={()=>{if(!busy)setMoneyStaff(null)}} title={moneyStaff?'Money · '+moneyStaff.name:'Staff money'}>
   <Field label="What happened?"><Select value={moneyKind} onChange={e=>setMoneyKind(e.target.value as typeof moneyKind)}><option value="advance">Give business money to hold / spend</option><option value="reimbursement">Reimburse personal pocket expense</option><option value="salary">Pay salary</option><option value="advance_return">Receive unused business money back</option></Select></Field>
   <Field label="Amount"><Input inputMode="decimal" value={moneyAmount} onChange={e=>setMoneyAmount(e.target.value)}/></Field>
   <Field label={moneyKind==='advance_return'?'Receive into':'Business money location'}><Select value={accountId} onChange={e=>setAccountId(e.target.value)}><option value="">Choose location</option>{activeAccounts.map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</Select></Field>
   <Field label="Note"><Textarea value={moneyNote} onChange={e=>setMoneyNote(e.target.value)}/></Field><Button full loading={busy} disabled={Number(moneyAmount)<=0||!accountId} onClick={recordMoney}><IndianRupee size={14}/> Record</Button>
  </Modal>
