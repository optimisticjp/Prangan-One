...
export default function BusinessDayClose() {
  ...
  const reopenNow = async () => { if (!reopen || reason.trim().length < 3) return; setBusy(true); try { await reopenDay(reopen.id, reason); toast.success('Day closing deleted / reopened'); setConfirmDelete(false); setReopen(null); setReason('') } catch (e) { toast.error(e instanceof Error ? e.message : 'Could not reopen day') } finally { setBusy(false) } }
  ...
  return <div className="space-y-3"><div><h1 className="text-[17px] font-bold text-navy-900">Day close</h1><p className="text-[11.5px] text-navy-400">Count actual cash and compare it with the ledger.</p></div>
    ...
    <Modal open={!!reopen} onClose={() => setReopen(null)} title="Reopen cash day"><Field label="Reason"><Input value={reason} onChange={e => setReason(e.target.value)} placeholder="Why is this closing being reopened?" /></Field><Button full variant="danger" disabled={reason.trim().length < 3} onClick={() => setConfirmDelete(true)}>Delete / reopen closing</Button></Modal><TypedConfirmModal open={confirmDelete} mode="DELETE" title="Delete day closing" body="This removes the closing so the day can be closed again. The reopen action stays in the activity audit log." busy={busy} onClose={() => setConfirmDelete(false)} onConfirm={reopenNow} />
  </div>
}
